import { setOrganizations, setOrganizationsError, setIsLoading, setSince } from 'src/redux/slices/organizations';
...
export const getOrganizations = (params: GetOrganizationsParams = {}): AppThunk => async (
  dispatch
) => {
  dispatch(setIsLoading(true))
  const { inPage = 10, since = 0 } = params;

  let url = `${GITHUB_API}/organizations?per_page=${inPage}&since=${since}`;
